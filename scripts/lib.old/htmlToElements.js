...
function throwInvalidHTMLError(message) {
...
export function createDomFromHtml(templateHtml = tmpHtml_pretty, placeholderData = {
    ...
    */
    ...
    function processHtml() {
        while(charI < templateHtml.length){
            console.log(charI, prevCharI, charI-1 != prevCharI, templateHtml.length);
            if(charI-1 != prevCharI){
    ...
    function handleData() {
        ...
        if (isStateActive("isWritingTagName")) {
            ...
            if(isStateActive("shouldCloseTagSpan")){
                if(datas.tagName !== dataVal){
                    throwInvalidHTMLError(`Received end tag '</${dataVal}>' on begin tag '<${datas.tagName}>'`);
                }
